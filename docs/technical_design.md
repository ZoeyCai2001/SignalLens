...
Event cluster APIs use the same local ranking preferences as the main feed, including configurable weights, preferred sources, and blocked sources. This keeps cross-source event discovery aligned with the user's personal source trust settings instead of exposing a separate global event view.
