...
Search and event cluster APIs use the same local ranking preferences as the main feed, including configurable weights, preferred sources, and blocked sources. Event clusters include deterministic MVP explanations and uncertainty notes derived from sources, shared topics, extracted tickers, classifier confidence, and timeline coverage; these fields provide the event-page explanation surface before optional LLM cluster summarization is enabled. The main feed, default search, and digest generation also honor language preferences when no explicit search language is supplied, keeping English/Chinese views aligned with the user's current reading focus instead of exposing separate global views.
