...
- Source ingestion from arXiv, Hacker News, Reddit public JSON, GitHub, Product Hunt, Hugging Face, selected RSS feeds, Alpha Vantage, and SEC EDGAR filings.
...
- WeChat public account connectors.
...
The MVP computes these as deterministic functions over stored fields, source metadata, and explicit local feedback. Feed and daily digest ranking also add a bounded cross-source confirmation boost when visible items share the same entity/event signature across sources, so repeated independent evidence can outrank slightly stronger isolated items without adding LLM cost. Feed responses expose the confirmation score and label, and both feed cards and item detail explanations mention the confirmation reason when it affects ranking. Stock timeline rows compare each stock-linked item with the closest stored event-date close-to-close move, label aligned, opposite, muted, or missing price reaction, and add uncertainty notes when a same-day move suggests the news may already be priced in or when price action moves against the inferred impact. Hidden items are removed from personal views but remain recoverable through a hidden-only feed query and `POST /api/feed/{item_id}/unhide`; marked-important items are ranked first and can be restored to normal priority through `POST /api/feed/{item_id}/unmark-important`; saved items receive a lightweight ranking boost so the main feed gradually reflects what the user keeps. Feed cards can also record explicit useful/not-useful item feedback through `POST /api/feed/{item_id}/mark-useful`, `POST /api/feed/{item_id}/mark-not-useful`, and `POST /api/feed/{item_id}/clear-feedback`, giving personalization a direct local signal without hiding or saving every item. The feed interest profile learns small bounded preferences from saved/important/useful and hidden/not-useful items, lifting matching sources/topics/entities and dampening repeated noisy matches without hiding them automatically. `GET /api/preferences/feedback-profile` exposes the learned profile as aggregate counts plus capped source, ticker, topic, product, and watchlist terms; the Settings dashboard renders those positive, negative, and watchlist signals in a Feedback Learning panel so the ranking effects remain inspectable without exposing raw article text or private notes. Expanded item details show personalization notes when a source, topic, product, company, or ticker matches prior positive or negative feedback, keeping recommendation learning auditable. Read/unread state is stored on user item actions, and saved-but-unread items drive the digest read-later section. Feed responses compute `social_signal_score` from free engagement metadata such as GitHub stars/stars-per-day/forks, Hacker News score/comment count, Reddit upvotes/comment count, Product Hunt votes/comments, Hugging Face downloads/likes, and generic public social metadata including likes, comments, views, collects/saves, and reposts/shares; ranking and digest ordering use that score without requiring an additional paid API or schema migration. Expanded feed details also expose the recognized public engagement metrics behind that score, using a constrained likes/comments/views/collects/reposts schema plus Reddit-specific upvote labels instead of leaking arbitrary raw metadata. Feed serialization preserves stored LLM `why_it_matters` text when available and otherwise generates a deterministic category/entity/signal fallback, including product use-case focus for product items, so item cards, details, digest, and exports keep the PRD explanation affordance even before paid summarization runs. Feed cards render compact uncertainty chips when low classifier confidence, lower source quality, unconfirmed market impact, or limited public signal should temper the item before the user opens details. Hacker News items also convert bounded top-comment previews into deterministic discussion summaries so developer conversation can be scanned without broad comment crawling or extra LLM spend. Reddit public JSON items receive deterministic discussion summaries from post metadata and self-post excerpts only, with an explicit no-private-content access note. Feed item details include deterministic uncertainty notes derived from classifier confidence, source quality, stock-impact linkage, summary availability, and manual-submission context so every expanded item can be audited before acting on it.

Per-item personal metadata is stored on `user_item_actions` rather than the source item itself. Expanded feed details can save a private personal note and normalized manual tags through `PATCH /api/feed/{item_id}/personal-metadata`, supporting the PRD saved-article notes and manual-tag workflow while keeping reading context user-local. The Saved Items panel surfaces notes, tags, read/read-later state, read timestamps, and read toggles inline, orders unread saved items before completed reading, and shows read-later/read counts so the saved list behaves like a lightweight reading queue. Tag chips run a saved-only manual-tag search so curated reading lists can become quick working filters. Saved items can also be exported through `GET /api/feed/saved/export/markdown` and `GET /api/feed/saved/export/json`; the dashboard can copy/download Markdown for reading or download structured JSON with source attribution, scores, read state, private notes, manual tags, feedback, and stored summaries, keeping the reading queue portable without another service. `DELETE /api/feed/{item_id}` permanently removes a feed item, its raw source text, personal action state, item alerts, and digest snapshots that still contain the item title or URL, while preserving LLM cost history with the item link cleared. The dashboard exposes this as a confirmed delete action on feed cards and hidden items for PRD privacy/compliance deletion, and the no-paid demo smoke verifier creates a temporary manual item, deletes it, and confirms the detail endpoint returns 404. Natural-language search treats "read later" and "to read" as saved-unread filters, matching the digest read-later section rather than a broad unread-only search.
...
Source quality is deterministic in the MVP. Structured research, official SEC filings, and official APIs receive the highest baseline credibility; community, RSS, manual, and experimental sources receive lower baseline scores. The stored `source_quality_score` is then used by ranking, importance, and digest selection without requiring an LLM call. SEC filing normalization also emits a deterministic stock-watch summary from filing metadata and receives a stronger stock-impact baseline for watched tickers, making official disclosures visible in stock briefings and digest ranking without spending LLM calls. The SEC connector keeps the default form set conservative at 8-K, 10-K, and 10-Q, while `SEC_FORMS` can add optional insider forms such as Form 4 for watched tickers. Manual Chinese/social submissions reuse the deterministic social-summary path, producing an English summary, inferred product/use-case labels, and an explicit manual-public-source note before any paid LLM enrichment is requested. The manual submission API and dashboard accept structured public engagement counts for likes, comments, collects/saves, reposts/shares, and views; pasted counts in notes remain a fallback. These counts are stored as raw metadata so social signal scoring can use user-provided X/Xiaohongshu-style traction evidence without crawling or paid social APIs.

Daily digest ordering uses a deterministic blended score across importance, relevance, source quality, classifier confidence, social signal, stock impact, and novelty. This keeps the morning briefing biased toward useful and trustworthy items while preserving high-priority user-marked items at the top. Digest generation also applies local visibility rules, so hidden items and blocked sources are excluded from date selection, section contents, active-alert context, source coverage, markdown export, and saved snapshots. Daily digest responses and markdown include compact active-alert summaries so urgent stock, product, social-trend, or cross-source signals are visible in the morning briefing without requiring a separate alert-panel scan. High social-signal digest items receive an explicit public-engagement label and markdown signal note, making it clear when Product Hunt/Xiaohongshu-style traction helped an item enter the daily brief.

Search and event cluster APIs use the same local ranking preferences as the main feed, including configurable weights, preferred sources, blocked sources, and optional PRD module scopes. Natural-language search returns inferred filters plus a deterministic local briefing for summary-style queries such as “summarize the most important semiconductor AI news this week,” using stored titles, summaries, sources, tickers, topics, and scores without spending LLM calls. Source inference covers arXiv, Hacker News, Reddit, GitHub, Hugging Face, Product Hunt, Alpha Vantage, SEC/EDGAR, and RSS phrasing, so source-specific queries can filter the archive without hand-setting advanced fields. Queries using viral, popular, or high-engagement language infer a minimum social-signal filter, so searches for popular AI photo tools or highly engaged social posts use the same public traction score as feed and digest ranking. Natural-language search also infers explicit AI-relevance review intent for phrases such as “not AI-related” or “needs relevance review,” surfaces that intent in dashboard chips, and routes product-style topic filters across both topic and product facets so Chinese social/product searches can match labels like `AI photo tool` even when the stored topic list is broader. The dashboard also exposes `min_social_signal_score` as an advanced search field for explicit traction filtering without needing special phrasing. Recent/latest searches infer a seven-day lower bound, while today/yesterday searches infer exact UTC calendar-day bounds and expose both `date_from` and `date_to` to the dashboard. Rolling-window searches such as past 24 hours, last two weeks, past month, and last 30 days anchor to the newest stored item date when available, keeping local demo and offline datasets searchable even when the collection date differs from the current clock. Event clusters include deterministic MVP explanations and uncertainty notes derived from sources, shared topics, extracted tickers, classifier confidence, and timeline coverage. Cluster responses expose `source_count`, `duplicate_item_count`, and `confirmation_level` so the dashboard can distinguish single-source candidates, repeated single-source coverage, normal cross-source confirmation, and stronger cross-source confirmation without an LLM call. Strong entity clusters use both the extracted ticker/product and a lightweight event signature from titles and topics, so unrelated same-ticker developments such as chip launches and earnings guidance do not collapse into one broad stock bucket. The dashboard can request an on-demand Kimi cluster explanation for expanded cluster evidence, making PRD-level LLM explanations explicit and cost-controlled instead of automatic. Cluster timeline rows show date, source, title, and importance so the PRD event timeline can be read without opening every evidence link. Feed item details surface stored LLM uncertainty notes from detailed summaries before appending deterministic audit flags, so PRD uncertainty review survives both model-backed and model-skipped paths. When a cluster has an extracted ticker, the DB-backed cluster list/detail responses attach the latest stock market snapshot for the first related ticker and a conservative event-date market reaction summary, so the dashboard can show price context alongside source evidence without another provider call. The main feed, default search, and digest generation also honor language preferences when no explicit search language is supplied, keeping English/Chinese views aligned with the user's current reading focus instead of exposing separate global views. Searches launched from AI Trends, Research, Products, AI Stocks, or Chinese Social stay inside that active module and update the module result cache so the visible list matches the returned search results; clearing search from a module reloads that module's ranked feed directly instead of temporarily falling back to a global feed refresh.

Alert generation applies the same trust posture and local visibility rules. Single-item alerts require enough classification confidence and source quality before a rule can fire, hidden feed items and blocked sources are excluded, and cross-source alerts require enough cluster confidence before a repeated signal is promoted as confirmed. Stock price-move alerts combine the latest daily close-to-close move for watched or rule-matched tickers with a stock-linked AI item, so dashboard urgency can reflect both news relevance and whether the price already reacted while keeping the non-financial-advice posture. System Readiness tracks the latest watched-stock price snapshot date, flags missing or stale price data for active stock watchlists, and can launch the Alpha Vantage price refresh action directly so stock-sensitive alerts and briefings are not reviewed against stale market context. Generic alert rules fold PRD secondary categories into their parent alert scopes, so research rules cover benchmark/evaluation, stock/company rules cover funding/M&A, and AI trend rules cover policy/regulation, infrastructure, open-source release, and tutorial/opinion items. Specialized stock-sensitive alert categories cover earnings/guidance mentions, analyst actions, supply-chain signals, and multi-source theme breakouts using deterministic term and topic matching over existing normalized items. Theme-breakout alerts require at least two high-quality sources inside a 24-hour window anchored to the newest matching topic item, matching the PRD "within 24 hours" breakout requirement without another model call. A social-trend alert category detects viral AI products, Chinese/social-platform signals, and high-engagement community launches from existing metadata such as Product Hunt votes, Hacker News activity, GitHub traction, public RSS language, inferred product entities, and PRD product use-case labels without requiring a paid social API; social alert reasons include compact public engagement counts such as votes, comments, likes, views, collects, and reposts when available. Alert rules can be disabled or snoozed until a specific timestamp; snoozed rules remain editable but are excluded from every alert generator until they resume. Quality metrics count total, enabled, active, and snoozed alert rules so readiness can distinguish missing alert output from paused or disabled rule configuration. Dashboard rule details can update names, descriptions, categories, severities, importance thresholds, stock-impact thresholds, tickers, and topics after creation, while backend guards keep required text fields named and clamp score thresholds to the valid range. Dismissed alerts remain queryable as history, while dashboard metrics continue to count active alerts only. Alert cards can record explicit useful/not-useful feedback, and quality metrics use that feedback to refine the PRD alert-usefulness proxy while retaining dismissal share as a fallback when no feedback exists. Alert reasons include the trust signals used for the decision so the user can audit why something was surfaced.
...
- `POST /api/feed/{item_id}/mark-useful`
- `POST /api/feed/{item_id}/mark-not-useful`
- `POST /api/feed/{item_id}/clear-feedback`
...
The repository root includes `pnpm setup:check`, backed by `python3 scripts/check_local_setup.py`, as a stdlib-only local readiness checker for Mac development. The direct Python command remains available before `pnpm` is installed. It verifies required project files, installed dependency folders, local API virtualenv executables, core command availability, configured environment variable names, likely iCloud-synced workspace paths, `.icloud` placeholder files, and macOS dataless project/Git metadata without printing secret values. Core gaps return a non-zero exit code, while recommended and optional integration gaps are warnings so budget-limited demo work can proceed without paid or optional APIs. System Readiness mirrors the same optional/free source posture with explicit badges for Kimi, GitHub, Alpha Vantage, SEC, Reddit, Product Hunt, and Chinese RSS so supported source families have visible setup state before the user runs ingestion.
...
Custom followed sources can run through reusable connectors when the access pattern is safe and structured. RSS sources with a `base_url` run through `RssConnector`; `arxiv_query` sources run through the free arXiv Atom API with configured category codes and keywords, preserving the PRD research-source workflow without paid access. `github_repository` sources with a GitHub URL run through the GitHub REST API for that specific repository, enabling the PRD source-watchlist requirement for followed GitHub projects without scraping. `product_topic` sources run through the official Product Hunt GraphQL API when `PRODUCT_HUNT_API_TOKEN` is configured and locally filter launches by configured topic terms, Product Hunt topic URLs, or the followed source name. The built-in Reddit source uses `REDDIT_USER_AGENT` and `REDDIT_SUBREDDITS` for public subreddit JSON search. Custom `reddit_community` sources parse subreddits from `r/...` URLs, use configured terms as the query, and store public post metadata plus short self-post excerpts only. `social_keyword` sources run only against user-provided public RSS/Atom feeds and locally filter entries by configured keywords, giving the MVP a compliant experimental path for Chinese/Xiaohongshu-style trend monitoring without login-protected scraping. X/Twitter account watch sources are manual-watch records in the MVP: they preserve attribution for manually submitted public links without making automated API calls or scraping restricted surfaces. The dashboard follow-source form captures URL, polling interval, rate-limit notes, and terms notes up front, and the same fields remain editable in Source Health. Full ingestion cycles run the built-in sources first and then enabled custom followed sources that are due according to their polling interval, so source-watchlist entries participate in scheduled collection without ignoring user-configured frequency. Source Health and run history expose per-run duration derived from recorded start and finish times, making ingestion latency visible without a new job telemetry table. Unexpected per-source exceptions are converted into failed cycle rows so later sources, alert generation, and daily digest snapshot saving still run. Newly followed sources can be removed before they collect items or run history; after that, disabling preserves historical attribution while stopping future ingestion.

Built-in source routes include manual actions for Hacker News, Reddit AI Communities, Alpha Vantage news, Alpha Vantage prices, SEC filings, arXiv, Chinese RSS, GitHub, Hugging Face, Product Hunt, selected RSS feeds, and the full ingestion cycle. The dashboard mirrors those routes in the toolbar so the user can run a free official filings refresh without waiting for the scheduler.

Source Health rows surface the latest run status, recent failure count, recent success rate, recent stored/fetched ratio, next due time, stale/overdue state, latest error text, collection mode, run support, raw-content storage policy, failure-handling guidance, and source-specific recovery hints so ingestion failures, duplicate-heavy sources, overdue sources, low-yield sources, and source compliance posture can be diagnosed directly from the dashboard instead of requiring terminal logs. Recovery hints name the relevant optional setup path where possible, such as `PRODUCT_HUNT_API_TOKEN`, `ALPHA_VANTAGE_API_KEY`, `SEC_USER_AGENT`, `REDDIT_USER_AGENT`, public subreddit URLs, or `CHINESE_RSS_FEEDS`; they remain derived from run status and source metadata rather than stored as database state. For retry visibility, `next_run_due_at` is based on the latest failed/skipped source-run attempt when available, while successful sources use `last_success_at`; this matches the scheduler's polling-window throttle after failed runs without hiding the last working run. Full-cycle and single-source run responses also summarize duration or source status, per-source store rates, attention flags, and recovery hints so the dashboard can quickly confirm whether a daily run stayed inside the PRD timing target and which connectors need a credential, rate-limit, configuration, or relevance review. System Readiness aggregates those source capabilities into total/enabled, runnable, manual-watch, and unconfigured followed-source counts beside source API cost metrics, making live collection coverage visible before a source run is attempted. Stale detection reuses the same polling-interval parser as the scheduled ingestion cycle, so Source Health and scheduler eligibility agree on text such as `hourly`, `daily`, or `6 hours`; disabled sources are not marked stale while paused. Source run history can be filtered to failed runs and scoped to a single source through the API and dashboard so connector issues can be triaged without scanning unrelated successful runs.

The dashboard also exposes PRD quality metrics through `GET /api/quality-metrics`, including recent item count, latest visible item timestamp and age, recent item counts by PRD module, covered module count, recent source count, dominant-source share, trusted-source coverage, low-quality item count, search-facet coverage, unfaceted item count, high-confidence classification coverage, low-confidence item count, high-value item count, high-value items per day, high-value unsummarized count, product-signal count, high-traction product-signal count, product-signal source count, event-cluster count, confirmed event-cluster count, cluster timeline evidence count, clustered recent-item count and share, relevance precision proxy, duplicate rate, summary coverage, summary quality proxy, thin-summary count, source failure rate, save/hide ratio, feedback action count, explicit item-feedback count and usefulness rate, manual submission count, manual enrichment gap count, stock, company, topic, and product watchlist counts, populated watchlist area count, saved read/read-later counts, alert rule readiness counts, alert dismissal rate, alert feedback count and usefulness rate, alert usefulness proxy, saved digest count, digest feedback count, digest feedback usefulness rate, digest usefulness proxy, the latest saved digest date, the latest saved digest age in days, and the latest saved digest item count. These metrics are deterministic local proxies over stored items, user actions, alerts, source runs, watchlists, clusters, and digest snapshots, giving the user a low-cost way to judge whether SignalLens is becoming more useful over time without adding analytics tracking or external services. Summary quality checks whether stored summaries are long enough to scan and whether high-value items include why-it-matters context or a detailed summary, keeping the PRD summary-quality metric visible without another model call. Digest usefulness falls back to saved snapshot freshness and item coverage, then blends in explicit useful/not-useful snapshot feedback when it exists; System Readiness shows the digest feedback sample count and usefulness rate beside the blended proxy, and the no-paid demo verifier marks the seeded digest useful to prove the full feedback loop. Explicit item usefulness is tracked separately from save/hide behavior so the user can train ranking without changing reading-queue state. Alert usefulness falls back to non-dismissed share when no alert feedback exists, then blends in explicit useful/not-useful alert feedback as the user reviews generated alerts. Product discovery is tracked separately from generic module coverage so missing product launches or missing public traction metadata can trigger Source Health review before the user has to manually check Product Hunt, GitHub, Hugging Face, or product RSS sources. Event-cluster readiness uses the same deterministic cluster builder as the dashboard, requires at least two related items, and counts cross-source confirmation without spending LLM budget. These remain local proxies rather than provider or behavioral analytics. The same response includes local quality findings with concrete recommendations and dashboard action targets for no recent ingestion, stale collection, thin PRD module coverage, thin source diversity, empty event clustering, thin cross-source confirmation, thin trusted-source coverage, thin search facets, thin watchlist coverage, empty product launch coverage, thin product traction signals, manual submissions needing enrichment, low classification confidence, low relevance precision, duplicate pressure, thin summary coverage, thin summary quality, unsummarized high-value items, missing personal feedback, read-later backlog, paused alert rules, high-value signals with no active alerts, noisy alert rules, source failures, missing, stale, or thin digest snapshots, or high LLM calls per recent item so System Readiness points toward the next operational fix and can jump directly to the relevant workflow, including the matching Source Health triage filter when a source issue is detected. Findings can expose explicit operations such as `cycle`, `llm:classify`, `llm:summarize`, `alerts:generate`, or `digest:save-snapshot`, letting the dashboard run a full ingestion cycle, run capped classification, run capped summarization, generate dashboard alerts, or save the latest digest snapshot directly from System Readiness while preserving the PRD cost-control posture. Quality-finding actions use the same workflow scroll targets as the checklist so the user lands on the feed, clusters, source health, digest, stock, alert, settings, or readiness panel that explains the next step.

System Readiness also renders a live PRD MVP checklist from `GET /api/mvp-checklist`, which derives from the same local metrics and translates feed freshness and coverage, source ingestion, LLM processing, watchlists, stock monitoring, search, digest freshness, alerts, and manual submissions into ready, partial, or needs-action states. The ranked dashboard row requires a newest visible item within 36 hours and at least 70% relevance precision before it is fully ready, turning the PRD daily-collection and relevance success metrics into visible run-cycle or source-tuning prompts when the feed is stale or too noisy. Stock monitoring readiness now requires watched tickers, recent watched-stock signals, and price context; the dashboard shows recent stock-signal count, high-impact stock-signal count, and watched ticker signal coverage so the PRD stock-news linkage path is visible without opening each stock detail. When watched tickers have no recent stock-linked items, quality findings recommend running a full ingestion cycle and reviewing finance/company sources before trusting stock alerts and briefings. Source ingestion readiness audits the named PRD connector families, including arXiv, Hacker News, Reddit, selected RSS, Alpha Vantage, Product Hunt, GitHub, Hugging Face, and Chinese RSS, instead of relying only on a generic enabled-source count. Checklist rows include direct dashboard actions where a local workflow exists, such as seeding demo data, running a full ingestion cycle, running capped classification, refreshing stock prices, saving a digest snapshot, generating alerts, opening the stock module, or opening the manual submission workflow. These actions also scroll to the relevant dashboard workflow panel so the checklist behaves like an operational launcher rather than a passive status grid. The dashboard keeps a local fallback checklist builder for older or temporarily unavailable backends, but the API contract is the authoritative path verified by the no-paid-API demo smoke check.
...
- Validate arXiv, Hacker News, Reddit public JSON, GitHub, Product Hunt, Hugging Face, RSS, and finance provider access.
