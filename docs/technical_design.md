...
Digest payload fields that were added after earlier snapshots, such as company watchlist context, use schema defaults so saved historical snapshots remain readable after backend upgrades. The dashboard Daily Digest panel can explicitly generate a fresh preview via `POST /api/digest/daily/generate`, copy markdown, and save the current digest as a snapshot. It shows the latest saved snapshots with dates, headlines, item counts, and markdown size so saved briefings are inspectable instead of only counted.
