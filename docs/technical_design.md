...
Source Health rows surface the latest run status, recent failure count, recent success rate, recent stored/fetched ratio, next due time, stale/overdue state, and latest error text so ingestion failures, duplicate-heavy sources, overdue sources, and low-yield sources can be diagnosed directly from the dashboard instead of requiring terminal logs. Full-cycle responses also summarize duration, successful sources, failed sources, and skipped sources so the dashboard can quickly confirm whether a daily run stayed inside the PRD timing target and which connectors need attention. Stale detection reuses the same polling-interval parser as the scheduled ingestion cycle, so Source Health and scheduler eligibility agree on text such as `hourly`, `daily`, or `6 hours`; disabled sources are not marked stale while paused. Source run history can be filtered to failed runs and scoped to a single source through the API and dashboard so connector issues can be triaged without scanning unrelated successful runs.
