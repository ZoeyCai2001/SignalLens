...
The repository root includes `pnpm setup:check`, a stdlib-only local readiness checker for Mac development. It verifies required project files, installed dependency folders, local API virtualenv executables, core command availability, and configured environment variable names without printing secret values. Core gaps return a non-zero exit code, while recommended and optional integration gaps are warnings so budget-limited demo work can proceed without paid or optional APIs.
